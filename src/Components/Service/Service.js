...
const Service = () => {
  ...
  useEffect(() => {
    fetch(`https://vast-ocean-05710.herokuapp.com/services/${serviceId}`)
      .then((res) => res.json())
      .then((data) => setService(data));
  }, []);
  const { _id, picture, title, price, description, rating, date, time } =
    service;
                ...
                <div className="mt-2">
                  ...
                  <div>
                    <p className="d-flex flex-row">
                      <span>{plane}</span> <span className="ms-3">{date}</span>
                    </p>
                  </div>
                  <hr />
                  <div>
                    <p className="d-flex flex-row">
                      <span>{clock}</span>{" "}
                      <span className="ms-3">{time} days</span>
                    </p>
                  </div>
                  <hr />
                  <div>
